...
## TL;DR
* Hana EVM is a smart contract written in Move and deployed to Sui blockchain.
* Hana EVM is not a blockchain and doesn't form a concensus. Its state is secured by Sui.
* Hana EVM is not a compiler.
* Hana EVM Module alone can't provide full EVM-compatibility. It must run with a sidecar we call [Proxy](proxy.md).

## Gateway and VM
`vm::vm` module in the source code interprets the EVM bytecode and handle EVM state. You can learn the way to interact with it from `vm::vm_gateway_test` module. 

## Remaining Issues
Move language doesn't have a feature that is equivalent to `view` function in Solidity. And, it's not supported at Move bytecode level either. This feature has been proposed by community and actively being developed by Mysten labs. Reference issue can be tracked [here](https://github.com/MystenLabs/sui/issues/4967).

## Opcodes Development Progress
Listed below are implemented opcodes. They are not fully tested yet. Document format is borrowed from [Kakarot Project](https://github.com/sayajin-labs/kakarot).
